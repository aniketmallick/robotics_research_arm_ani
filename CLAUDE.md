...
2. Every joint target passes through `safety.clamp_action()`. Two clamp profiles: **`policy`** — base ±90°, other joints ±60°, wrist_roll ±150° (pure rotation, collision-free; calibrated range is ±180°), gripper 0–100% — used for every LLM/IK-originated target; **`recorded`** — the calibrated physical range minus a 2° margin — used only when replaying human teleop recordings, which are ground truth and may legitimately exceed policy limits. Both profiles share the max-speed limit. All motion is interpolated (20–30 Hz), never a raw jump to target. **Limits govern where the arm may be COMMANDED TO GO, not where it may BE**: the start-pose sanity check refuses motion only when the measured start lies outside the calibrated PHYSICAL range ± 2° (an encoder/calibration fault) — a parked pose outside the policy envelope is a legal start, and lerp-from-measured-start guarantees every step moves toward the target envelope, never further out. Return-to-entry recovery targets use the `recorded` profile (entry poses are measured reality; policy must not clip them). Send-boundary backstop clamp = physical limits; it should never visibly fire — warn if it does.
...
- **Grasp — DEMO PATH = TAUGHT ZONES (Plan C, ratified after stage-4 calibration proved too fragile).** N=5 marked table spots; the operator teleop-records one working pick macro per zone (reusing the stage-2 record/replay engine); the arm replays the macro for the chosen zone. Vision's job is IDENTITY, not coordinates: Gemini locates the named object's pixel and it is assigned to the nearest zone by a coarse, click-once **pixel** zone map (no robot-frame calibration, no homography, no IK, no riser/verticality problem — the human demonstrates a grasp that works). Objects sit at the marked spots. All trust gates stay intact because they are about identity and judgment, not millimetres.
  - **STRETCH (not the demo path):** homography + IK + hover→descend→close→lift for pick-anywhere. Kept in the repo (`eyes.py` fully reused; `calibrate.py`/`kinematics.py`/`grasp.py` hover path intact). Only attempt if time is left over. Geometry note if revived: near-vertical approach only exists at z≲0; use a ~10–12cm riser + negative `ARMANI_TABLE_HEIGHT_M`, keep ±60° limits, tip-method calibration must beat 15px (it did not on the first pass — clicks too imprecise + gripper lean).
- **IK ladder:** Plan A = lerobot 0.5.2 `RobotKinematics` (placo + `armani/data/so101_kin.urdf`), CONFIRMED working stage 4 — single soft-QP step with no success flag, so `kinematics.py` iterates to rest and verifies every solution by FK, and feeds the solver our policy limits so solutions are policy-legal by construction. (Note: the google robotics-pointing-sample uses MJCF + hand-rolled damped-least-squares and is position-only — a separate approach, not this; its position-only IK corroborates that full top-down orientation is unattainable on this 5-DOF arm.) → Plan B: phosphobot REST `/move/absolute` → Plan C: taught zones (5 marked spots, one recorded pick macro each; VLM only chooses the zone). Plan switch decisions belong to the human architect, not you. **placo is installed in the lerobot env (numpy pinned 2.2.6, cmeel-urdfdom 4.0.1 + cmeel-tinyxml2 10.0.0); teleop imports verified unchanged — a deliberate, ratified exception to "don't touch that env."**
